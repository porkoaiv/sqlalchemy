...
class VerticaUuid(sqltypes.UUID[sqltypes._UUID_RETURN]):
    ...
    if TYPE_CHECKING:
        ...
        @overload
        def __init__(
            self: VerticaUuid[_python_UUID], as_uuid: Literal[True] = ...
        ) -> None: ...

        @overload
        def __init__(
            self: VerticaUuid[str], as_uuid: Literal[False] = ...
        ) -> None: ...
...
VerticaMacAddr8 = MACADDR8


class MONEY(sqltypes.TypeEngine[str]):
    r"""Provide the Vertica MONEY type.
...
class OID(sqltypes.TypeEngine[int]):
    """Provide the Vertica OID type."""

    __visit_name__ = "OID"


class REGCONFIG(sqltypes.TypeEngine[str]):
    """Provide the Vertica REGCONFIG type.
...
class TSQUERY(sqltypes.TypeEngine[str]):
    """Provide the Vertica TSQUERY type.
...
class REGCLASS(sqltypes.TypeEngine[str]):
    """Provide the Vertica REGCLASS type.
...
class TIMESTAMP(sqltypes.TIMESTAMP):
    """Provide the Vertica TIMESTAMP type."""
...
class TSVECTOR(sqltypes.TypeEngine[str]):
    """The :class:`_vertica.TSVECTOR` type implements the Vertica
...
class CITEXT(sqltypes.TEXT):
    """Provide the Vertica CITEXT type.
