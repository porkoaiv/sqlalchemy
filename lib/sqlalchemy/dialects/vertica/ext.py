...
class aggregate_order_by(expression.ColumnElement):
    """Represent a Vertica aggregate order by expression.

    E.g.::

        from sqlalchemy.dialects.vertica import aggregate_order_by
...
class ExcludeConstraint(ColumnCollectionConstraint):
    ...
    Defines an EXCLUDE constraint as described in the `Vertica
    documentation`__.

    __ https://www.vertica.org/docs/current/static/sql-createtable.html#SQL-CREATETABLE-EXCLUDE
    ...
    create_drop_stringify_dialect = "vertica"
    ...
    def __init__(self, *elements, **kw):
        ...
        :param ops:
          ...
          :ref:`vertica_ops <vertica_operator_classes>`
          ...
          .. seealso::
            ...
            :ref:`vertica_operator_classes` - general description of how
            Vertica operator classes are specified.
...
def array_agg(*arg, **kw):
    """Vertica-specific form of :class:`_functions.array_agg`, ensures
    return type is :class:`_vertica.ARRAY` and not
...
class to_tsvector(_regconfig_fn):
    """The Vertica ``to_tsvector`` SQL function.

    This function applies automatic casting of the REGCONFIG argument
    to use the :class:`_vertica.REGCONFIG` datatype automatically,
    and applies a return type of :class:`_vertica.TSVECTOR`.

    Assuming the Vertica dialect has been imported, either by invoking
    ``from sqlalchemy.dialects import vertica``, or by creating a Vertica
    engine using ``create_engine("vertica...")``,
    :class:`_vertica.to_tsvector` will be used automatically when invoking
...
class to_tsquery(_regconfig_fn):
    """The Vertica ``to_tsquery`` SQL function.

    This function applies automatic casting of the REGCONFIG argument
    to use the :class:`_vertica.REGCONFIG` datatype automatically,
    and applies a return type of :class:`_vertica.TSQUERY`.

    Assuming the Vertica dialect has been imported, either by invoking
    ``from sqlalchemy.dialects import vertica``, or by creating a Vertica
    engine using ``create_engine("vertica...")``,
    :class:`_vertica.to_tsquery` will be used automatically when invoking
...
class plainto_tsquery(_regconfig_fn):
    """The Vertica ``plainto_tsquery`` SQL function.
    ...
    ``from sqlalchemy.dialects import vertica``, or by creating a Vertica
    engine using ``create_engine("vertica...")``,
    :class:`_vertica.plainto_tsquery` will be used automatically when
...
class phraseto_tsquery(_regconfig_fn):
    """The Vertica ``phraseto_tsquery`` SQL function.
    ...
    ``from sqlalchemy.dialects import vertica``, or by creating a Vertica
    engine using ``create_engine("vertica...")``,
    :class:`_vertica.phraseto_tsquery` will be used automatically when
...
class websearch_to_tsquery(_regconfig_fn):
    """The Vertica ``websearch_to_tsquery`` SQL function.
    ...
    ``from sqlalchemy.dialects import vertica``, or by creating a Vertica
    engine using ``create_engine("vertica...")``,
    :class:`_vertica.websearch_to_tsquery` will be used automatically when
...
class ts_headline(_regconfig_fn):
    """The Vertica ``ts_headline`` SQL function.
    ...
    ``from sqlalchemy.dialects import vertica``, or by creating a Vertica
    engine using ``create_engine("vertica...")``,
    :class:`_vertica.ts_headline` will be used automatically when invoking
    ...
    def __init__(self, *args, **kwargs):
        ...
        # https://www.vertica.org/docs/current/textsearch-controls.html#TEXTSEARCH-HEADLINE
        if len(args) < 2:
