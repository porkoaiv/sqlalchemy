...
class array(expression.ExpressionClauseList[_T]):
    """A Vertica ARRAY literal.

    This is used to produce ARRAY literals in SQL expressions, e.g.::

        from sqlalchemy.dialects.vertica import array
        from sqlalchemy.dialects import vertica
        from sqlalchemy import select, func

        stmt = select(array([1,2]) + array([3,4,5]))

        print(stmt.compile(dialect=vertica.dialect()))
...
class ARRAY(sqltypes.ARRAY):
    ...
    The :class:`_vertica.ARRAY` type is constructed in the same way
    ...
    than one dimension::
        ...
        mytable = Table("mytable", metadata,
                Column("data", vertica.ARRAY(Integer, dimensions=2))
            )

    The :class:`_vertica.ARRAY` type provides all operations defined on the
    core :class:`_types.ARRAY` type, including support for "dimensions",
    indexed access, and simple matching such as
    :meth:`.types.ARRAY.Comparator.any` and
    :meth:`.types.ARRAY.Comparator.all`.  :class:`_vertica.ARRAY`
    class also
    provides Vertica-specific methods for containment operations, including
    :meth:`.vertica.ARRAY.Comparator.contains`
    :meth:`.vertica.ARRAY.Comparator.contained_by`, and
    :meth:`.vertica.ARRAY.Comparator.overlap`, e.g.::

        mytable.c.data.contains([1, 2])

    Indexed access is one-based by default, to match that of Vertica;
    for zero-based indexed access, set
    :paramref:`_vertica.ARRAY.zero_indexes`.

    Additionally, the :class:`_vertica.ARRAY`
    type does not work directly in
    conjunction with the :class:`.ENUM` type.  For a workaround, see the
    special type at :ref:`vertica_array_of_enum`.

    .. container:: topic

        **Detecting Changes in ARRAY columns when using the ORM**

        The :class:`_vertica.ARRAY` type, when used with the SQLAlchemy ORM,
    ...
    .. seealso::
        ...
        :class:`_vertica.array` - produces a literal array value.
    ...
    def __init__(
        ...
        :param as_tuple=False: Specify whether return results
          ...
          as psycov2 return lists by default. When tuples are
        ...
        :param dimensions: if non-None, the ARRAY will assume a fixed
         ...
         Note that Vertica arrays are always implicitly "non-dimensioned",
         meaning they can store any number of dimensions no matter how
         they were declared.

        :param zero_indexes=False: when True, index values will be converted
         between Python zero-based and Vertica one-based indexes, e.g.
