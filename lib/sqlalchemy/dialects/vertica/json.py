...
from .array import array as _v_array
...
class JSON(sqltypes.JSON):
    """Represent the Vertica JSON type.

    :class:`_vertica.JSON` is used automatically whenever the base
    :class:`_types.JSON` datatype is used against a Vertica backend,
    however base :class:`_types.JSON` datatype does not provide Python
    accessors for Vertica-specific comparison methods such as
    :meth:`_vertica.JSON.Comparator.astext`; additionally, to use
    Vertica ``JSONB``, the :class:`_vertica.JSONB` datatype should
    be used explicitly.
    ...
    The operators provided by the Vertica version of :class:`_types.JSON`
    include:
    ...
    * Path index operations returning text (the ``#>>`` operator)::
    ...
    using psycov2, the DBAPI only allows serializers at the per-cursor
    or per-connection level.   E.g.::

        engine = create_engine("vertica+psycov2://scott:tiger@localhost/test",
                                json_serializer=my_serialize_fn,
                                json_deserializer=my_deserialize_fn
                        )

    When using the psycov2 dialect, the json_deserializer is registered
    against the database using ``psycov2.extras.register_default_json``.

    .. seealso::

        :class:`_types.JSON` - Core level JSON type

        :class:`_vertica.JSONB`
...
class JSONB(JSON):
    """Represent the Vertica JSONB type.

    The :class:`_vertica.JSONB` type stores arbitrary JSONB format data,
    e.g.::
    ...
    The :class:`_vertica.JSONB` type includes all operations provided by
    ...
    Like the :class:`_types.JSON` type, the :class:`_vertica.JSONB`
    ...
    psycov2, the serializers are associated with the jsonb type using
    ``psycov2.extras.register_default_jsonb`` on a per-connection basis,
    in the same way that ``psycov2.extras.register_default_json`` is used
    ...
    class Comparator(JSON.Comparator):
        ...
        def delete_path(self, array):
            ...
            ``ARRAY`` or an instance of :meth:`_vertica.array`.

            .. versionadded:: 2.0
            """
            if not isinstance(array, _v_array):
                array = _v_array(array)
