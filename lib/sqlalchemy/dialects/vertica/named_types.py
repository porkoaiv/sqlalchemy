# dialects/vertica/named_types.py
...
class NamedType(sqltypes.TypeEngine):
    ...
    def create(self, bind, checkfirst=True, **kw):
        ...
        :param checkfirst: if ``True``, a query against
         the Vertica catalog will be first performed to see
    ...
    def _check_for_name_in_memos(self, checkfirst, kw):
        ...
        if "_ddl_runner" in kw:
            ...
            type_name = f"v_{self.__visit_name__}"
...
class ENUM(NamedType, type_api.NativeForEmulated, sqltypes.Enum):
    ...
    support for Vertica's ``CREATE TYPE`` and ``DROP TYPE``.

    When the builtin type :class:`_types.Enum` is used and the
    :paramref:`.Enum.native_enum` flag is left at its default of
    True, the Vertica backend will use a :class:`_vertica.ENUM`
    ...
    When using :class:`_types.Enum` or :class:`_vertica.ENUM`
    ...
    :class:`_vertica.ENUM` independently, and associate it with the
    ...
    def __init__(
        ...
        **kw,
        ...
        """Construct an :class:`_vertica.ENUM`.
        ...
        :param create_type: Defaults to True.
         ...
         :meth:`~.vertica.ENUM.create`
         or :meth:`~.vertica.ENUM.drop`
         are called directly.
         Setting to ``False`` is helpful
         when invoking a creation scheme to a SQL file
         without access to the actual database -
         the :meth:`~.vertica.ENUM.create` and
         :meth:`~.vertica.ENUM.drop` methods can
        ...
        if native_enum is False:
            util.warn(
                ...
                "sqlalchemy.dialects.vertica.ENUM datatype; this type "
    ...
    def adapt_emulated_to_native(cls, impl, **kw):
        """Produce a Vertica native :class:`_vertica.ENUM` from plain
        :class:`.Enum`.
    ...
    def create(self, bind=None, checkfirst=True):
        ...
        Vertica CREATE TYPE, no action is taken.
    ...
    def drop(self, bind=None, checkfirst=True):
        ...
        If the underlying dialect does not support
        Vertica DROP TYPE, no action is taken.
    ...
    def get_dbapi_type(self, dbapi):
        """dont return dbapi.STRING for ENUM in Vertica, since that's
        a different type"""
...
class DOMAIN(NamedType, sqltypes.SchemaType):
    r"""Represent the DOMAIN Vertica type.
    ...
    See the `Vertica documentation`__ for additional details

    __ https://www.vertica.org/docs/current/sql-createdomain.html
    ...
    def __init__(
        ...
        **kw: Any,
        ...
        :param check: CHECK clause specify integrity constraint or test
          ...
          Differently from Vertica, only a single check clause is
          currently allowed in SQLAlchemy.
        :param schema: optional schema name
        :param metadata: optional :class:`_schema.MetaData` object which
         this :class:`_vertica.DOMAIN` will be directly associated
        :param create_type: Defaults to True.
