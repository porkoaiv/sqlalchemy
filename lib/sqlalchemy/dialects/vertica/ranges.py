...
class Range(Generic[_T]):
    """Represent a Vertica range.

    E.g.::

        r = Range(10, 50, bounds="()")

    The calling style is similar to that of psycov and psycov2, in part
    ...
    def _get_discrete_step(self) -> Any:
        ...
        # See
        # https://www.vertica.org/docs/current/rangetypes.html#RANGETYPES-DISCRETE
        # for the rationale
...
class MultiRange(List[Range[_T]]):
    ...
    This is useful when operating on literal multi-ranges::
        ...
        from sqlalchemy.dialects.vertica import MultiRange, Range
    ...
    .. seealso::
        ...
        - :ref:`vertica_multirange_list_use`.
    """
...
class AbstractRange(sqltypes.TypeEngine[_T]):
    ...
    def adapt(
        ...
        **kw: Any,
        ...
        For example ``INT4RANGE().adapt(_Psycov2NumericRange)`` should
        produce a type that will have ``_Psycov2NumericRange`` behaviors
        ...
        if (
            ...
            # visit_abstract_range_impl() method in the Vertica compiler.
...
class AbstractSingleRange(AbstractRange[Range[_T]]):
    """Base for Vertica RANGE types.

    These are types that return a single :class:`_vertica.Range` object.

    .. seealso::

        `Vertica range functions <https://www.vertica.org/docs/current/static/functions-range.html>`_
    ...
    def _resolve_for_literal(self, value: Range[Any]) -> Any:
        ...
        if isinstance(spec, int):
            # v is unreasonably picky here: the query
            # "select 1::INTEGER <@ '[1, 4)'::INT8RANGE" raises
            # "operator does not exist: integer <@ int8range" as of v 16
            if _is_int32(value):
                return INT4RANGE()
            else:
                return INT8RANGE()
...
class AbstractMultiRange(AbstractRange[Sequence[Range[_T]]]):
    """Base for Vertica MULTIRANGE types.

    these are types that return a sequence of :class:`_vertica.Range`
    objects.
    ...
    def _resolve_for_literal(self, value: Sequence[Range[Any]]) -> Any:
        ...
        if isinstance(spec, int):
            ...
            # "operator does not exist: integer <@ int8multirange" as of v 16
